...
int main(){
    ...
    draw(val,dir,x,y);
    //switches direction from start point and does the same pattern reflected on other side
    if(dir==0){dir=2;x-=2;}
    else if(dir==1){dir=3;y+=2;}
    else if(dir==2){dir=0;x+=2;}
    else if(dir==3){dir=1;y-=2;}
    draw(val,dir,x,y);
    gotoxy(50,50);
    return 0;
}
